...
- 新增了独立的 `Share links / QR` 菜单，可以只挑指定协议的分享链接和二维码单独查看，也可以直接导出面向客户端的片段
...
- 同一个菜单还可以把选中的链接导出成 `v2rayN` 这种一行一个 URI 的订阅文本，生成 dae 可直接粘贴的 `node { ... }` 片段，或导出一份最小可跑的 dae 配置；`ShadowTLS`、`SS2022` 这类不在当前 dae 实测兼容矩阵里的条目会自动跳过
- 内置证书管理：
...
- `dae` 目前只纳入已经在 `hk` 上实流量跑通的协议：经典 `Shadowsocks`、`VMess`、`Trojan`、`VLESS`、`VLESS-Reality`、`Hysteria2/HY2`、`TUIC`
- `ShadowTLS` 不在 `dae` 兼容目标内，`SS2022` 则因为当前 `dae v1.0.0` 在实测里拒绝了导出的 AEAD-2022 cipher，所以也会被自动跳过
